 ...
 * Beendet in der Vorlesung am 25.01.18.
 *
...
//Dreht die Reihenfolge um T13.1b
  public void reverse() {
    ...
    while(pos!=null){
      Link<E> tmp = pos.getNext();
      pos.setNext(pos.getPrev()); 
      pos.setPrev(tmp);
      pos = tmp;
      //Wenn dass letzte objekt ist irgendwann null = while erfüllt
  ...
  private static class MyIterator<E> implements ListIterator<E> {
    ...
    private MyIterator(MyList2<E> liste) {
      this.liste = liste;
      this.positionsZeiger   = liste.first;
    ...
    public E previous() {
      if (positionsZeiger == null) { // Wir sind total am Ende!
        ...
        letztGelesenesPos = positionsZeiger; //positionsZeiger wandert eins weiter
        positionsZeiger = pre;    //seigt auf die nächste Pos
        index--;
        if (pre == null) return null;     //Positionszeiger ist nach dem return immer noch an der selben stelle
    ...
    public void remove() {
        ...
        if (nachfolger != null) {
          ...
          liste.last = vorgaenger;
        }
        if (vorgaenger != null) {
          vorgaenger.setNext(nachfolger);
        } else {
          liste.first = nachfolger;
        }
    ...
    @Override
    public void add(E e) {
      if (positionsZeiger == null) { liste.addLast(e); }
      else {
