 ...
 *
 *
 * Modified by Phillip Heidegger (2007.09.21), (2008.09.11)
 *)
...
module String :
  sig
    ...
    *)
    ...
    val lazy_concat : string Lazy.t -> string Lazy.t -> string Lazy.t
    (** [lazy_concat s1 s2] concats the two suspensions if the result
        is forced. You can concatenate the two lazy strings, and get
        the garantie, that is will done only once, and only if the 
        hole string is forced. *)

	(** {2 Older Functions} *)

	(** Please refer to the 
        {{:http://caml.inria.fr/pub/docs/manual-ocaml/libref/List.html}Ocaml Manual} 
