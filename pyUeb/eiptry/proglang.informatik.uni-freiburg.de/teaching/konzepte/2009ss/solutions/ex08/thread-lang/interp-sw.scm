(module interp-sw (lib "eopl.ss" "eopl")
  ...
  (require "store.scm")
  (require "scheduler-sw.scm")
  (require "semaphores-sw.scm")     
    ...
    (lambda (cont val)
      (if (time-expired?)
        (begin
          ;; SW
          (place-on-ready-queue!
           (make-thread (lambda () (apply-cont cont val))))
          (run-next-thread))
        (begin
                   ...
                   (a-thread 
                    ...
                    (lambda ()
                      (apply-procedure proc1
                                       (list-val (list (num-val tid) (num-val cur-tid)))
                                       (end-subthread-cont)))))
                  (apply-cont saved-cont (num-val tid)))))

            (wait-cont (saved-cont)
              (wait-for-mutex
                (expval->mutex val)
                (make-thread (lambda () (apply-cont saved-cont (num-val 52))))))

            (signal-cont (saved-cont)
              (signal-mutex
                (expval->mutex val)
                (make-thread (lambda () (apply-cont saved-cont (num-val 53))))))
