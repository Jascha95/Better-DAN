(module tests-sw (lib "eopl.ss" "eopl")
...
let tid11 = -1 in
let tid12 = -1 in
let parent-tid11 = -1 in
let parent-tid12 = -1 in
let tid21 = -1 in
let tid22 = -1 in
let parent-tid21 = -1 in
let parent-tid22 = -1 in
let mut = mutex() in
let end1 = mutex() in
let end2 = mutex() in
let thread1 = proc(tids)
                begin
                  wait(mut);
                  set tid11 = car(tids);
                  set parent-tid11 = car(cdr(tids));
                  signal(mut);
                  yield();
                  wait(mut);
                  set tid12 = car(tids);
                  set parent-tid12 = car(cdr(tids));
                  signal(mut);
                  yield();
                  signal(end1)
                end
in
let thread2 = proc(tids)
                begin
                  wait(mut);
                  set tid21 = car(tids);
                  set parent-tid21 = car(cdr(tids));
                  signal(mut);
                  yield();
                  wait(mut);
                  set tid22 = car(tids);
                  set parent-tid22 = car(cdr(tids));
                  signal(mut);
                  yield();
                  signal(end2)
                end
...
"
      ...
      (thread-ids-1 ,(mt-prog "tid11") 1)
      (thread-ids-2 ,(mt-prog "tid12") 1)
      (thread-ids-3 ,(mt-prog "tid21") 2)
      (thread-ids-4 ,(mt-prog "tid22") 2)
      (thread-ids-5 ,(mt-prog "parent-tid11") 0)
      (thread-ids-6 ,(mt-prog "parent-tid12") 0)
      (thread-ids-7 ,(mt-prog "parent-tid21") 0)
      (thread-ids-8 ,(mt-prog "parent-tid22") 0)
