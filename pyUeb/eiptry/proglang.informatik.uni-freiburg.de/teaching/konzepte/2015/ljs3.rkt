...
;; _                     _     _  _    ____
;;| |_ _  _ _ __  ___ __| |___| |(_)__|__ /
;;|  _| || | '_ \/ -_) _` |___| || (_-<|_ \
;; \__|\_, | .__/\___\__,_|   |_|/ /__/___/
;;     |__/|_|                 |__/        

(define-extended-language typed-ljs3 typed-ljs2
  
  ;; field names
  ((o p q) variable-not-otherwise-mentioned)
  ...
  ;; expressions
  ((E F G) .... ((p : E) ...) (get E p) (set E p F))
  
   ;; evaluation context
  ((C D) .... ((o : V) ... (p : C) (q : E) ...) (get C p) (set C p F) (set V p C))
)


;; test sntax
;(redex-match typed-ljs3 E (term ()))

;(redex-match typed-ljs3 E (term ((p : 1))))
;(redex-match typed-ljs3 E (term ((p : (1 + 1)))))
;(redex-match typed-ljs3 E (term ((p : x))))

;(redex-match typed-ljs3 E (term ((o : 1) (p : 2))))

;(redex-match typed-ljs3 E (term (get ((p : 1)) p)))
;(redex-match typed-ljs3 E (term (get ((p : 1)) q)))

;(redex-match typed-ljs3 E (term (set ((p : 1)) p 2)))
;(redex-match typed-ljs3 E (term (set ((p : 1)) q 2)))
...
(define-metafunction typed-ljs3  
  ;indom : V x -> Bool
;  [(indom ((o : U) ... (p : V) (q : W) ...) p) #t]
  [(indom? any p) ,true]
)



(define typed-reduction2
  (extend-reduction-relation typed-reduction
   typed-ljs3
   (--> (in-hole C (get ((o : U) ... (p : V) (q : W) ...) p))
        (in-hole C V)
        Get
   )
   (--> (in-hole C (get ((o : U) ...) p))
        (in-hole C Udef)
        Get-Udef
        (side-condition (not (member (term p) (term (o ...))))) 
   )
   (--> (in-hole C (set ((o : U) ... (p : V_1) (q : W) ...) p V_2))
        (in-hole C ((o : U) ... (p : V_2) (q : W) ...))
        Update
   )
   (--> (in-hole C (set ((o : U) ...) p V))
        (in-hole C ((o : U) ... (p : V)))
        Add
        (side-condition (not (member (term p) (term (o ...)))))
   )
...
;; test reduction
;(traces typed-reduction2 (term (get () p)))
;(traces typed-reduction2 (term (get ((p : 1)) p)))
;(traces typed-reduction2 (term (get ((p : 1)) q)))

;(traces typed-reduction2 (term (set () p 1)))
;(traces typed-reduction2 (term (set ((p : 1)) p 2)))
;(traces typed-reduction2 (term (set ((p : 1)) q 2)))
...
(define-judgment-form
 ...
 [(types Γ E Bool) 
  ...
  [(types Γ ((o : E) ... (p : F) (q : G) ...) ((o : τ_1) ... (p : τ_2) (q : τ_3) ...))
   -----------------------
   (types Γ (get ((o : E) ... (p : F) (q : G) ...) p) τ_2)]

  [(types Γ ((o : E) ... ) ((o : τ_1) ... ))
   -----------------------
   (types Γ (get ((o : E) ... ) p) Udef)]
  
  [(types Γ F_1 τ) 
   (types Γ ((o : E) ... (p : F) (q : G) ...) ((o : τ_1) ... (p : τ_2) (q : τ_3) ...))
   -----------------------
   (types Γ (set ((o : E) ... (p : F) (q : G) ...) p F_1) ((o : τ_1) ... (p : τ) (q : τ_3) ...))]

  [(types Γ F τ) 
   (types Γ ((o : E) ... ) ((o : τ_1) ...))
   -----------------------
   (types Γ (set ((o : E) ... ) p F) ((o : τ_1) ... (p : τ)))]
...
;; test judgement
;(judgment-holds (types · () ()))
;(judgment-holds (types · ((p : 1)) ((p : Int))))
;(judgment-holds (types · ((p : (1 + 1))) ((p : Int))))

;(judgment-holds (types · (get ((p : 2)) p) Int))
;(judgment-holds (types · (get ((p : 2)) q) Udef)) ;; not type rule for udef

;(judgment-holds (types · (set ((p : 2)) p 1) ((p : Int))))
;(judgment-holds (types · (set ((p : 2)) p true) ((p : Bool))))
;(judgment-holds (types · (set ((p : 2)) q true) ((p : Int) (q : Bool))))
